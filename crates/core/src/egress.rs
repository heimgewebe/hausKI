...
impl EgressGuard {
    ...
    pub fn from_policy(policy: &RoutingPolicy) -> Result<Self, EgressGuardError> {
        ...
        let Some(egress_value) = mapping.get(serde_yml::Value::from(KEY_EGRESS)) else {
        ...
        let default_action = egress_map
            .get(serde_yml::Value::from(KEY_DEFAULT))
        ...
        if let Some(allow_value) = egress_map.get(serde_yml::Value::from(KEY_ALLOW)) {
...
mod tests {
    ...
    fn policy_from_yaml(yaml: &str) -> RoutingPolicy {
        RoutingPolicy(serde_yml::from_str(yaml).unwrap())
    }
