...
#[serde(transparent)]
pub struct RoutingPolicy(pub serde_yml::Value);

pub type RoutingRule = serde_yml::Value;
pub type RoutingDecision = serde_yml::Value;
...
pub fn load_limits<P: AsRef<Path>>(path: P) -> Result<Limits> {
    ...
    let limits = serde_yml::from_str(&content).map_err(|e| {
...
pub fn load_models<P: AsRef<Path>>(path: P) -> Result<ModelsFile> {
    ...
    let models = serde_yml::from_str(&content).map_err(|e| {
...
pub fn load_routing<P: AsRef<Path>>(path: P) -> Result<RoutingPolicy> {
    ...
    let routing = serde_yml::from_str(&content).map_err(|e| {
...
pub fn load_flags<P: AsRef<Path>>(path: P) -> Result<FeatureFlags> {
    ...
    let mut flags: FeatureFlags = serde_yml::from_str(&content).map_err(|e| {
...
mod tests {
    ...
    fn routing_policy_with_explicit_default_and_no_rules() {
        ...
        let allow_key = serde_yml::Value::String("allow".into());
        assert_eq!(
            mapping.get(&default_key),
            Some(&serde_yml::Value::String("deny".into()))
        );
